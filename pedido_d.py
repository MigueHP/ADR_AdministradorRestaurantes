...
class PedidoDaO:
    ...
    _ID = 'SELECT id_pedido_stock from pedido_stock ORDER BY id_pedido_stock DESC LIMIT 1'
    _INSERTAR = 'INSERT INTO pedido_stock(fecha_pedido, id_proveedor, id_empleado) VALUES(%s,%s,%s)'
    _ACTUALIZAR = 'UPDATE pedido_stock SET fecha_pedido=%s, id_proveedor=%s, id_empleado=%s WHERE id_pedido_stock=%s'
    _ELIMINAR = 'DELETE FROM pedido_stock WHERE id_pedido_stock=%s'
    ...
    def insertar(cls, pedido):
        with CursorDelPool() as cursor:
            valores =  (pedido.fecha, pedido.id_proveedor_, pedido.id_empleado_)
            cursor.execute(cls._INSERTAR, valores)
            log.debug(f'Persona insertada: {pedido}')
